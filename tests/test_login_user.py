...
class TestLoginUser:
    ...
    def test_login_user(self, login_user):
        assert login_user.status_code == StatusCode.OK and login_user.json()['success'] == True
    ...
    def test_login_with_invalid_data(self, email, password, create_user):
        ...
        assert response.status_code == StatusCode.UNAUTHORIZED and response.text == '{"success":false,"message":"email or password are incorrect"}'
